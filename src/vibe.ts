...
function readFencePath(info: string) {
  ...
  const barePath = info.match(
    /^([^\s]+\.(?:html|css|js|cjs|mjs|jsx|ts|tsx|json|md|svg|txt|py|cs|csproj|sln|vb|vbproj|fs|fsproj|java|kt|kts|gradle|go|rs|swift|php|rb|r|lua|dart|ex|exs|erl|hrl|clj|cljs|scala|sql|ps1|sh|bat|cmd|toml|yml|yaml|xml|env|gitignore))$/i,
  );
  return barePath?.[1] ?? '';
}
